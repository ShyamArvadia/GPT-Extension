...
(() => {
  ...
  async function getHtml({ query: query2, timerange, region }) {
    const formData = new URLSearchParams({
      q: query2.slice(0, 495),
      // DDG limit
      df: timerange,
      kl: region
    });
...
Make sure the URL is correct.`,
  ...
  import_webextension_polyfill5.default.declarativeNetRequest.updateDynamicRules({
    addRules: [
      {
        id: 1,
        priority: 1,
  ...
  function update_origin_for_ddg_in_firefox() {
    import_webextension_polyfill5.default.webRequest.onBeforeSendHeaders.addListener(
      (details) => {
